...
Implemented timeout support for agent runs with the following changes:

### Files Modified:
1. **internal/state/manager.go** - Added `TimeoutAt` and `TimeoutReason` fields to `AgentState`
2. **internal/config/config.go** - Added `Timeout` and `IterTimeout` config options with TOML support
3. **internal/agent/config.go** - Added `Timeout` field to agent `Config` struct
4. **internal/agent/runner.go** - Added `RunWithContext` method with context-based timeout using `exec.CommandContext`
5. **cmd/run.go** - Added `--timeout` and `--iter-timeout` flags with full support for:
   - CLI flags
   - Config file defaults
   - Detached mode (timeout values passed to child process)
   - Single iteration mode
   - Multi-iteration mode with total timeout context
   - Exit code 124 on timeout (GNU timeout convention)
6. **cmd/inspect.go** - Added display of timeout info (remaining time, expired status, timeout reason)

### Features Implemented:
- `--timeout` flag for total run timeout
- `--iter-timeout` flag for per-iteration timeout
- Config file support (`timeout` and `iter_timeout` in TOML)
- Time formats: `30s`, `5m`, `2h`, `1h30m` (Go duration format)
- Exit code 124 when terminated by timeout
- Timeout state tracking (`TimeoutAt`, `TimeoutReason`)
- `swarm inspect` shows timeout info
- Works in foreground and detached modes
