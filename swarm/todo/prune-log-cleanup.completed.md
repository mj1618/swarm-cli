...
Implemented the `--logs` flag for `swarm prune` as specified:

1. Added `pruneLogs` flag variable
2. Updated Long description and Example in command definition
3. Updated confirmation prompt to mention log files when `--logs` is specified
4. Added log file deletion logic that:
   - Only runs when `--logs` flag is set
   - Skips agents with empty LogFile (foreground agents)
   - Handles missing files gracefully (ignores `os.IsNotExist` errors)
   - Warns on permission errors but continues
   - Tracks count of deleted log files
5. Updated summary message to show log file count when applicable
6. Added `--logs` flag registration in `init()`

All tests pass and the help output correctly shows the new flag.
